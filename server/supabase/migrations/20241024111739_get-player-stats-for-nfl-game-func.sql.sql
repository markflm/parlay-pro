create or replace function getPlayerStatsForGameNfl(game_public_id_param text)
  RETURNS TABLE(player_public_id text, player_name text, player_position text, team_name text, game_name text, game_public_id text, game_isi text,
  passing_comp int, passing_att int, passing_yds int, passing_td int, passing_int int, rushing_att int, rushing_yds int, rushing_td int, receiving_rec int, receiving_yds int, receiving_tgt int, receiving_td int
   ...
   game_internal_id bigint;
   team_one_param bigint;
   team_two_param bigint;
  begin
    select played_at into game_timestamp from games where publicid::text = game_public_id_param;
    select id into game_internal_id from games where publicid::text = game_public_id_param;
    select team_one into team_one_param from games where id = game_internal_id;
    select team_two into team_two_param from games where id = game_internal_id;

    return query select p.publicId::text as player_public_id, p.name as player_name, p.position, t.name as team_name, g.name as game_name, g.publicId::text as game_public_id, g.intraseason_indicator as game_isi, gl.passing_comp, gl.passing_att, gl.passing_yds, gl.passing_td, gl.passing_int, gl.rushing_att, gl.rushing_yds, gl.rushing_td, gl.receiving_rec, gl.receiving_yds, gl.receiving_tgt, gl.receiving_td from players p join teams t on p.team = t.id join games g on t.id = g.team_one OR t.id = g.team_two
